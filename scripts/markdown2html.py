...
# 版本号
VERSION = "8.0.0-field-notes-theme"
...
def normalize_asset_url(url, assets_name="assets"):
    """将 Markdown 中的本地资源路径转换为站点中的资源路径。"""
    url = url.strip()
    if not url or url.startswith(("#", "//")) or re.match(r"^[a-zA-Z][a-zA-Z0-9+.-]*:", url):
        return url

    url = url.replace("\\", "/")
    assets_root = assets_name.replace("\\", "/").strip("/")
    if not assets_root:
        return url
    ...
    for prefix in (f"content/{assets_root}/", f"{assets_root}/"):
        if url.startswith(prefix):
            asset_relative_path = url[len(prefix) :]
            break

    if asset_relative_path is None:
        return url

    # 保留已有的百分号编码，同时编码中文、空格和括号等 URL 特殊字符。
    encoded_path = quote(asset_relative_path, safe="/:@-._~!$&'*+,;=%")
    return f"{assets_root}/{encoded_path}"


def convert_obsidian_image_embeds(markdown, assets_name="assets"):
    """将 Obsidian 的 ![[path|alt]] 图片嵌入语法转换为标准 Markdown。"""

    def replace_embed(match):
        target = match.group(1).strip()
        image_path, separator, alt_text = target.partition("|")
        image_path = image_path.strip()
        if not image_path:
            return match.group(0)

        alt_text = alt_text.strip() if separator else ""
        image_url = normalize_asset_url(image_path, assets_name)
        return f"![{alt_text}]({image_url})"

    return re.sub(r"!\[\[([^\]\r\n]+)\]\]", replace_embed, markdown)
...
class HighlightRenderer(mistune.HTMLRenderer):
    ...
    def __init__(self, assets_name="assets"):
        super().__init__()
        self.assets_name = assets_name

    def image(self, text, url, title=None):
        url = normalize_asset_url(url, self.assets_name)
        return super().image(text, url, title)


def get_markdown_parser(assets_name="assets"):
    return mistune.create_markdown(
        renderer=HighlightRenderer(assets_name=assets_name),
        plugins=["table", "url", "strikethrough", "task_lists", "math"],
    )
...
def process_single_file(input_path, output_dir, jinja_template, assets_name="assets"):
    ...
    full_markdown_body = convert_obsidian_image_embeds(full_markdown_body, assets_name)
    ...
    # 3. 转 HTML
    parser = get_markdown_parser(assets_name)
...
def build_site(input_dir, output_dir, assets_name, tpl_article_path, tpl_index_path):
    ...
    try:
        ...
        # 注册自定义测试：用于 selectattr('tags', 'containing', tag) 语法
    ...
    for md_file in md_files:
        meta = process_single_file(md_file, output_path, tpl_article, assets_name)
        articles_metadata.append(meta)

    generate_index_page(articles_metadata, output_path, tpl_index)

    output_assets_path = output_path / assets_name
    if output_assets_path.exists():
        shutil.rmtree(output_assets_path)
    shutil.copytree(input_path / assets_name, output_assets_path)
